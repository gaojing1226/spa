$(function(){
    $('form :input').blur(function(){
        ...
        if($(this).is('.rectangle__height')){
            if(this.value=="" ){
                var errorMsg4 = '高度不能为空!';
                $(this).next().after('<div class="formtips onError4">'+errorMsg4+'</div>');
            }
            else if(this.value<0 ){
                var errorMsg5 = '高度必须大于0!';
                $(this).next().after('<div class="formtips onError5">'+errorMsg5+'</div>');
            }
        }

    })

    $('.rectangle__width').keypress(function(e){
        validateKey(e);
    });
        
    $('.rectangle__height').keypress(function(e){
        validateKey(e);
    });
